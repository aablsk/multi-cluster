...
provider "kubernetes" {
  alias                  = "gke_az2"
  host                   = "https://${module.gke_az2.endpoint}"
...
module "gke_az2" {
  ...
  ip_range_services       = local.network[local.cluster_name_az2].ip_range_services
  ...
  node_pools = [
    {
      ...
      node_locations     = "${var.region_az2}-a"
      node_count         = 3
      disk_size_gb       = 100
...
resource "google_gke_hub_membership" "gke_az2" {
  count = var.enable_mcs == true ? 1 : 0 # create this resource only if mcs is enabled

  provider      = google-beta
  project       = var.project_id
  membership_id = "${local.cluster_name_az2}-membership"
  endpoint {
    gke_cluster {
      resource_link = "//container.googleapis.com/${module.gke_az2.cluster_id}"
    }
  }
  authority {
    issuer = "https://container.googleapis.com/v1/${module.gke_az2.cluster_id}"
  }
}
